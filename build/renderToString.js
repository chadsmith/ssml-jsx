...
function render(node, options = {}) {
  ...
  if (Array.isArray(node)) {
    return node.map(child => render(child, { ...options,
      root: false
    })).join('');
  }

  if (typeof node.elementName === 'function') {
    return render(node.elementName({ ...node.attributes,
  ...
  return children.length ? `<${node.elementName}${attributes}>${children.map(child => render(child, { ...options,
    root: false
  })).join('')}</${node.elementName}>` : `<${node.elementName}${attributes}/>`;
}